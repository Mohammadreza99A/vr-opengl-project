...
void Trunk::init(GLuint pid, GLuint _vao)
{
    create_positions();
...
void Trunk::add_sub_trunks_lsystem(Transform t, uint level, uint max_level, bool is_end_point)
{
    if (level >= max_level)
    {
        transf.push_back(t);
...
float Trunk::lerp(float val, float min, float max)
{
    return (1 - val) * min + val * max;
}

void Trunk::add_sub_trunks_lsystem_random(Transform t, uint level, uint max_level, bool is_end_point)
{
    if (level >= max_level)
    {
...
void Trunk::add_sub_trunks_fractal(Transform t, uint level, uint max_level)
{
...
std::vector<glm::mat4> Trunk::get_transf()
{
...
uint Trunk::indices_to_draw()
{
    return nb_indices;
}

std::vector<glm::mat4> Trunk::get_end_point_matrices()
{
    std::vector<glm::mat4> ret;
